...
interface RestUsuario {
    ...
    suspend fun reportarUsuario(@Header("Authorization") token:String):Response<List<Usuario>>
    ...
    suspend fun deleteUsuario(@Header("Authorization") token:String, @Path("id") id:Long):Response<MsgGeneric>

    @PUT("/asis/usuario/editar/{id}")
    suspend fun actualizarUsuario(@Header("Authorization") token:String, @Path("id") id:Long, @Body actividad: Usuario): Response<Usuario>
